...
public class ScenarioOfDSLAM implements ScenerioOfComponent {
	String id;
	...
	public ScenarioOfDSLAM(WorkerComponent wc) {
		try {
			...
			comm = commApi
					.createCommunication(
							...
							ResourceHelper.getString("/config.properties",
									"pInputQueue"),
							"JMSType='DSLAM'  or JMSType = 'SIGNALSTOP'",
							MyFormatter.class);
	...
	public void start() {
		...
		try {
			...
			do {
				if (responseReceived.getType().equals("DSLAM")) {
			...
			} while (responseReceived.getType().equals("DSLAM"));
	...
	public void repiqueMyObject(Object object) {
	}

	public void putMyDataInDatagrid(ObjectForDSLAM object) {
		...
		if (type_evenement.equals("a") || type_evenement.equals("m")) {
			...
			System.out.println(usedatagridapi.getMyData(
					String.valueOf(object.getId()), new ObjectForDSLAM())
					.getId());
